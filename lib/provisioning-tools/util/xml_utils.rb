...
class Util::VirshDomainXmlDiffer
  ...
  def initialize(expected, actual)
    ...
    machine = get_attribute(actual, '/domain/os/type/@machine')
    @differences.push "Bad attribute. Element /domain/os/type has incorrect attribute \"machine=#{machine}\"." if machine == 'pc-1.0'
  ...
  def get_attribute(document, xpath)
    REXML::XPath.match(document, xpath).map(&:value).map(&:strip).first
  end
