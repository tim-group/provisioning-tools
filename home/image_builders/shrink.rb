...
define "shrink" do
  ...
  run("loopback devices") do
    ...
    blockcount = `dumpe2fs -h /dev/mapper/#{spec[:loop0]}p1 | grep -F "Block count:" | awk -F ':' '{ print $2 }' | \
      sed 's/ //g'`.chomp.to_i

    blocksize = `sudo dumpe2fs -h /dev/mapper/#{spec[:loop0]}p1 | grep -F "Block size:" | awk -F ':' '{ print $2 }' | \
      sed 's/ //g'`.chomp.to_i
